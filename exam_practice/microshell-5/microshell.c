...
#define ERR_EXEC "error: cannot exec "
...
void    get_cmd_end(t_data **data, char **argv, int i)
{
...
int handle_cd(t_data **data, char **argv)
{
    if ((*data)->end - (*data)->start != 2)
        return (show_error(ERR_CD1, NULL));
    if (chdir(argv[1]))
        return (show_error(ERR_CD2, argv[1]));
    return (EXIT_SUCCESS);
}
