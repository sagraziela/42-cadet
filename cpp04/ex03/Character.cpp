...
void Character::equip(AMateria *m)
{
	...
	std::cout << "It was NOT possible to add " << m->getType() << " to " << this->_name << "'s inventory, cause it is already full.\n";
}
...
void Character::unequip(int idx)
{
	if (idx >= 0 && idx < SIZE && this->_inventory[idx] != NULL)
	{
		...
		std::cout << "IDX " << idx << " of " << this->_name << "'s inventory was unequiped.\n";
	}
	std::cout << "FAILED to unequip " << this->_name << std::endl;
}


void Character::use(int idx, ICharacter& target)
{
	if ((idx >= 0 && idx < SIZE) && this->_inventory[idx])
		this->_inventory[idx]->use(target);
	else
		std::cout << "It is NOT possible to use any AMateria. " << this->_name << "'s inventory is empty.\n";
}
